...
import static com.techcia.security.Constants.HEADER_STRING;
import static com.techcia.security.Constants.TOKEN_PREFIX;
    ...
    protected void doFilterInternal(HttpServletRequest req, HttpServletResponse res, FilterChain chain) throws IOException, ServletException {
        ...
        if (header != null && header.startsWith(TOKEN_PREFIX)) {
            ...
            try {
                ...
                role = jwtTokenUtil.getAuthoritiesFromToken(authToken);
            } catch (IllegalArgumentException e) {
                logger.error("an error occured during getting username from token", e);
            } catch (ExpiredJwtException e) {
                logger.warn("the token is expe6yhired and not valid anymore", e);
